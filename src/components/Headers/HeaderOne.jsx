  ...
  return (
    <header className="bg-black text-white px-6 py-3 h-[64px] flex items-center justify-between border-b border-neutral-800">
      ...
      <nav className="hidden md:flex space-x-6 text-base font-medium">
        {navItems.map((item) => (
          <div key={item.name} className="flex items-center gap-1 hover:text-gray-300 cursor-pointer">
            <span>{item.name}</span>
      ...
      <div className="flex items-center space-x-8">
        <Search size={21} className="hover:text-gray-300 cursor-pointer" />
        <Sparkles size={21} className="hover:text-gray-300 cursor-pointer" />
      </div>
    </header>
  );
};
